...
class TestTracker : public ::testing::Test {
 public:
  TestTracker() : tracker_params_(), tracker_(nullptr) {
    ...
    if (FLAGS_display) {
      window_ = std::make_unique<cv::viz::Viz3d>("Test KiTracker");
...
TEST_F(TestTracker, geometricOutlierRejection2d2d) {
  ...
  for (auto test_conf : test_configurations) {
    ...
    for (int t = 0; t < repeat_times; t++) {
      ...
      KiTracker tracker(trackerParams, stereo_camera_->getOriginalLeftCamera());
...
TEST_F(TestTracker, geometricOutlierRejection2d2dGivenRotation) {
  ...
  KiTracker tracker(tracker_params, stereo_camera_->getOriginalLeftCamera());
  // Start with the simplest case:
  // Noise free, no outlier, non-planar
...
TEST_F(TestTracker, getPoint3AndCovariance) {
  ...
  tie(f_ref_i_actual, cov_ref_i_actual) = KiTracker::getPoint3AndCovariance(
...
TEST_F(TestTracker, findOutliers) {
  // Normal case:
  {
    ...
    KiTracker::findOutliers(matches_ref_cur, inliers, &outliers_actual);
...
TEST_F(TestTracker, FindMatchingKeypoints) {
  ...
  KiTracker::findMatchingKeypoints(*ref_frame, *cur_frame, &matches_ref_cur);
...
TEST_F(TestTracker, FindMatchingStereoKeypoints) {
  ...
  vector<pair<size_t, size_t>> matches_ref_cur;
  KiTracker::findMatchingStereoKeypoints(
...
TEST_F(TestTracker, PnPTracking) {
  ...
  //! Create pnp tracker
  tracker_ = std::make_unique<KiTracker>(tracker_params_,
